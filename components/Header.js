...
import { Menu, X, User, ShoppingCart, LogOut, Settings, Gamepad2 } from 'lucide-react'
...
export default function Header() {
  ...
  const navigation = [
    ...
    { name: 'Monitoring Tools', href: '/monitoring-tools' },
    { name: 'Community', href: '/community' },
    { name: 'Games', href: '/games', icon: Gamepad2 }, // 🎮 NEW GAMES SECTION
            ...
            {navigation.map((item) => (
              <Link
                ...
                className={`text-gray-700 hover:text-primary-600 font-medium transition-colors flex items-center space-x-1 ${
                  item.name === 'Games' ? 'text-purple-600 hover:text-purple-700' : ''
                }`}
              >
              ...
              <div className="relative">
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-50">
                    ...
                    <Link
                      href="/games"
                      className="flex items-center space-x-2 px-4 py-2 text-purple-700 hover:bg-purple-50"
                      onClick={() => setIsUserMenuOpen(false)}
                    >
                      <Gamepad2 className="w-4 h-4" />
                      <span>Games & Learning</span>
                    </Link>
                    {user.role === 'admin' && (
                      <Link
                        href="/admin"
                        className="flex items-center space-x-2 px-4 py-2 text-red-700 hover:bg-red-50 font-medium"
                        onClick={() => setIsUserMenuOpen(false)}
                      >
          ...
          <nav className="flex flex-col space-y-4">
            ...
            {user && (
              <div className="flex flex-col space-y-2 pt-4 border-t">
                ...
                <Link
                  href="/games"
                  className="text-purple-700 hover:text-purple-800 font-medium flex items-center space-x-2"
                  onClick={() => setIsMenuOpen(false)}
                >
                  <Gamepad2 className="w-4 h-4" />
                  <span>Games & Learning</span>
