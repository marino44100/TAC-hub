...
export default function MonitoringToolsPage() {
    ...
    const renderManualHealthForm = () => ( <
            ...
            /label> {
            selectedImage && ( <
                p className = "text-sm text-green-600 mt-2" > ✓{ selectedImage.name }
                selected < /p>
            )
        } <
        /div>
    ...
    div className = "space-y-3" > {
            submissions.forestHealth.slice(0, 3).map((submission) => ( <
                ...
                span className = "text-sm text-gray-500" > { new Date(submission.timestamp).toLocaleDateString() } <
                /span> < /
                div > <
                ...
                submission.photos > 0 && < span > 📷{ submission.photos }
                photos < /span>} < /
                div > {
                    submission.notes && ( <
    ...
    switch (activeTab) {
        case 'forest-health':
            return ( <
                ...
                <
                ForestHealthAnalyzer onAnalysisComplete = {
                    (result) => {
                ...
                <
                div className = "border-t border-gray-200 pt-8" > { renderManualHealthForm() } <
                /div> < /
                div >
            )
        case 'weather-tracker':
            return ( <
                WeatherTracker onObservationComplete = {
                    (observationData) => {
                        // Save weather observation to submissions
                        const newSubmission = {
                            id: Date.now(),
                            ...observationData,
                            timestamp: new Date().toISOString()
                        ...
                        const updatedSubmissions = {
                            ...submissions,
                            weather: [newSubmission, ...submissions.weather]
                        }
        ...
        case 'wildlife-counter':
            return ( <
                WildlifeCounter onSightingComplete = {
                    (sightingData) => {
                        // Save wildlife observation to submissions
                        const newSubmission = {
                            id: Date.now(),
                            ...sightingData,
                            timestamp: new Date().toISOString()
                        ...
                        const updatedSubmissions = {
                            ...submissions,
                            wildlife: [newSubmission, ...submissions.wildlife]
                        }
...
return ( <
        ...
        AI - powered and traditional tools
